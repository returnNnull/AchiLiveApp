...
class AchievementObjectViewModel(private val categoryId: String) : ViewModel(), ViewModelProvider.Factory {
    ...
    private val _itemsState  = MutableStateFlow<ListUiState<AwardSchemeDTO>>(ListUiState.Loading())
    ...
    private fun init(categoryId: String) {
        viewModelScope.launch {
            try {
                val result = awardSchemeDataSource.where("categoriesId", categoryId).getOrThrow()
